...
* :class:`STRATIMINER_OT_copy_prompt` — copies the StratiMiner
  extraction prompt (v5.0, single file with 5 typed sheets) to the
...
class STRATIMINER_OT_copy_prompt(bpy.types.Operator):
    """Copy the StratiMiner extraction prompt (v5.0) to clipboard"""
    bl_idname = "stratiminer.copy_prompt"
    bl_label = "Copy StratiMiner Prompt"
    bl_description = (
        "Copy the StratiMiner extraction prompt (v5.0 unified schema) to "
